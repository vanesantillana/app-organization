    ...
    <div className="item" onClick={() => addChildrens()}>
      <Row justify="center">
        <Col flex="300px">
          ...
          >
            {isHiredThisMonth(item) && (
              <Tag color="success"> Recién contratado </Tag>
            )}
            {salaryIncrease && <Tag color="gold">Salario Incrementado</Tag>}
            <p>Área: {item.area}</p>
            <p>
              Salario: {salaryIncrease && salaryIncrease + " - "}
              {item.salary}
            </p>
          </Card>
        </Col>
  ...
  return (
    <TreeNode
      label={
        <NodeCard
          item={myItem}
          addChildrens={addChildren}
          salaryIncrease={salaryIncrease}
        />
      }
    >
      {myItem.children &&
        myItem.children.map((child) => (
          <Node item={child} dataByMonth={dataByMonth} allData={allData} />
        ))}
    </TreeNode>
  );
};
...
const Organigram = ({ allData, selectedDate }) => {
  const dataByMonth = allData.filter((item) =>
    isSameMonth(item.month, selectedDate)
  );
  ...
  return (
    <Tree
      ...
      label={
        <div className="item">Organigrama {getNameMonth(selectedDate)}</div>
      }
    >
...
const CustomTree = ({ data, selectedDate }) =>
  !!data ? (
    <Organigram allData={data} selectedDate={selectedDate} />
  ) : (
